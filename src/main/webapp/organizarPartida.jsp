...
<body>
	<c:choose>
		<c:when test="${cookie.tipo.value != 'centro'}">
		...
		<c:otherwise>
			...
			<form action="ProcesarPartidaServlet" method="post">
				<label>Fecha de activación:</label> <input type="date"
					name="fechaActivacion" required /><br /> <br /> <label>Nombre
					de la clase / descripción:</label> <input type="text" name="nombreClase"
					maxlength="100" required /><br /> <br /> <label>Escenario:</label>
				<select name="escenario">
					<option value="instituto">Instituto</option>
					<option value="redes">Redes Sociales</option>
					<option value="videojuegos">Videojuegos</option>
				</select><br /> <br /> <label>Idioma:</label> <select name="idioma">
					<option value="es">Español</option>
					<option value="en">Inglés</option>
					<option value="eu">Euskera</option>
				</select><br /> <br /> <input type="submit"
					value="Generar Código de Acceso" />
			</form>
		</c:otherwise>
	</c:choose>
</body>
